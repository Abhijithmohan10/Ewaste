...
if($res!=0)
{
	$bt="select * from bank where acc_no='$acc_no' and ownername='$str' and ifsccode='$ifsc'";
	$gt=$obj->GetSingleRow($bt);
	...
	if($amt_src>=$sum)
	{
		$client_amt=$gt['amount'];
		$new_cl_amt=$client_amt+$sum;
		$new_admin_amt=$amt_src-$sum;
		//echo "okk";
		$up_admin_qry="update bank set amount='$new_admin_amt' where acc_no='1234567890178952' and ownername='ADMIN' and ifsccode='1111'";
		...
		{
			//$date=date('Y-m-d');
			 $up_sch_emp="update schedule_employee set sch_status='completed' where req_id='$req_id' and sch_status='update'";
			...
			echo $obj->alert("Transaction Successful","../index.php");
		}
		else
		{
